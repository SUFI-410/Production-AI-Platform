...
async def _validate_upload(
    ...
    allowed_content_types = _ALLOWED_CONTENT_TYPES.get(
        suffix
    )
    ...
    if suffix == ".md":
        ...
        if not markdown.strip():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=(
                    "The uploaded Markdown file is empty."
                ),
            )
...
async def upload_document(
    file: Annotated[
        ...
        File(
            description=(
                "PDF or Markdown document belonging to "
    ...
    document_type: Annotated[
        ...
        Form(
            description=(
                "Business role of the uploaded document."
            ),
        ),
    ] = DocumentType.OTHER,
) -> DocumentRecord:
    ...
    filename, content = await _validate_upload(
        file
    )
    ...
    try:
        ...
        logger.exception(
            "Unable to persist uploaded document."
        )
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Unable to remove orphaned uploaded "
                "document after database failure."
            )

        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                "Unable to create the document record."
            ),
        ) from None
